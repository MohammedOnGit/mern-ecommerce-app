...
function ProductDetailsDialog({
  ...
  const canAddToCart = isActive && availableStock > 0;

  const isInWishlist = wishlistItems.some(
    (item) =>
      item.product?._id === _id ||
      item.product?.productId === _id ||
      item.productId === _id
  );
  ...
  const incrementQuantity = () => {
    if (quantity < availableStock) {
      ...
      toast.warning(`Only ${availableStock} item(s) available`);
    }
  };
          ...
          <div className="p-6 overflow-y-auto">
            ...
            <Tabs value={selectedTab} onValueChange={setSelectedTab}>
              ...
              <TabsContent value="specs" className="mt-4 text-sm space-y-1">
                ...
                <p>Brand: {brand}</p>
                <p>Stock: {availableStock} / {totalStock}</p>
              </TabsContent>
